...
final class FeaturedChromeTint {
    ...
    static func sampleURL(for item: WorkItem?) -> URL? {
        ...
        return processed(filename: filename, fallback: item.thumbnailURL?.absoluteString)
    }

    static func sampleURL(for detail: WorkDetail?) -> URL? {
        guard let detail else { return nil }
        return processed(filename: detail.featuredStillFilename, fallback: nil)
    }

    private static func processed(filename: String?, fallback: String?) -> URL? {
